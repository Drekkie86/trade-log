...
class ControlPlaneStatus:
    ...
    def ready(self) -> bool:
        return all(
            (
                self.release_identity_state == "PASS",
                self.core_services_state == "PASS",
                self.supervisor_state == "HEALTHY",
                self.research_progress_state
                in {"PASS", "INFO"},
                self.resource_policy_state == "PASS",
            )
        )
...
def collect_control_plane_status(
    *,
    ...
    deployed_commit, identity_state = (
    ...
    core_state = (
        ...
        if all(
            state == "active"
            for state in core_services.values()
        )
        else "FAIL"
    )
    ...
    observed_raw = supervisor.get(
        "observed_at"
    )
    observed_at = (
        None
        if observed_raw is None
        else str(observed_raw)
    )

    research_state, research_detail = (
        _research_progress(
            supervisor
        )
    )
    ...
    return ControlPlaneStatus(
        ...
        release_identity_state=identity_state,
        core_services=core_services,
        core_services_state=core_state,
        supervisor_state=supervisor_state,
        supervisor_observed_at=observed_at,
        research_progress_state=research_state,
        research_progress_detail=research_detail,
        resource_policy_state=resource_state,
        resource_policy_passed=passed,
        resource_policy_total=total,
    )