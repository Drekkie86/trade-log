...
def verify(connection: sqlite3.Connection) -> None:
    if version(connection) != EXPECTED_AFTER:
        raise RuntimeError("Schema did not reach v17.")

    columns = {
        row["name"]: row
        for row in connection.execute(
            "PRAGMA table_info(shadow_mark_observations);"
        ).fetchall()
    }
    ...
    for required in {
        ...
        if required not in columns:
            raise RuntimeError(
                f"Missing v17 shadow-mark column: {required}"
            )
    ...
    bad = connection.execute(
        ...
        WHERE measurement_role <>
              'INDEPENDENT_LEG_LIQUIDATION_STRESS'
           OR outcome_eligible <> 0;
        """
    ).fetchone()["count"]
    if bad:
        raise RuntimeError(
            f"v17 found {bad} historical shadow mark(s) with "
            "unexpected measurement semantics."
        )
...
def main() -> int:
    print("Christiania - real database migration to v17")
    print("---------------------------------------------")
    print()
    print(
        "This migration labels existing shadow marks as independent-leg "
        "liquidation stress evidence and marks them outcome-ineligible. "
        "It does not modify or delete existing mark values."
    )
    print()
    ...
    if current == EXPECTED_AFTER:
        ...
        print("Database is already valid v17.")
        return 0
    ...
    print("Database schema: v17")
    print("SQLite integrity_check: ok")
    print("Foreign-key check: clean")
    print("Shadow-mark measurement semantics: explicit")
    return 0
