...
from migrate_real_database_to_v9 import (
    connect,
    sqlite_backup,
    stamp,
)
...
MIGRATION = (
    ROOT
    / "migrations"
    / "010_hypothesis_scanner_evidence.sql"
)
...
def verify(
    ...
    required = {
        ...
        "trg_hypothesis_scanner_runs_no_update",
        "trg_hypothesis_scanner_evaluations_no_update",
    }

    missing = required - names
    if missing:
        raise RuntimeError(
            f"Missing v10 objects: {sorted(missing)}"
        )
...
def main() -> int:
    print(
        "Christiania - real database migration to v10"
    )
    print(
        "-------------------------------------------"
    )
    ...
    rehearsal = (
        ROOT / "trade_log_v10_rehearsal.db"
    )
    if rehearsal.exists():
        rehearsal.unlink()
    ...
    backup = (
        ROOT
        / f"trade_log_before_v10_{stamp()}.db"
    )
