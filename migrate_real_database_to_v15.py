...
MIGRATION_PATH = PROJECT_ROOT / "migrations" / "015_scanner_join_performance.sql"
EXPECTED_BEFORE = 14
EXPECTED_AFTER = 15
...
def sqlite_backup(source: Path, destination: Path) -> None:
    ...
    A raw filesystem copy of a WAL-mode database can silently omit
    committed data still sitting in the -wal sidecar file. This uses
    sqlite3's own backup() method, which is correct regardless of
    journal mode.
    """
...
def verify(connection: sqlite3.Connection) -> None:
    if version(connection) != EXPECTED_AFTER:
        raise RuntimeError("Schema did not reach v15.")
    ...
    for required in {
        "idx_listing_reference_scanner_join",
    }:
        if required not in objects:
            raise RuntimeError(f"Missing v15 schema object: {required}")

    # The index must actually cover all six join columns in the order the
    # scanner's query needs, or the migration didn't do what it claims.
    index_info = connection.execute(
        "PRAGMA index_info(idx_listing_reference_scanner_join);"
    ).fetchall()
    indexed_columns = [row["name"] for row in index_info]
    expected_columns = [
        "research_run_id",
        "provider",
        "underlying",
    ...
    if indexed_columns != expected_columns:
        raise RuntimeError(
            "idx_listing_reference_scanner_join does not cover the "
            f"expected columns in order. Found: {indexed_columns}"
        )
...
def main() -> int:
    print("Christiania - real database migration to v15")
    print("---------------------------------------------")
    print()
    print(
        "This migration adds an index only. It does not modify or "
    ...
    if current == EXPECTED_AFTER:
        ...
        try:
            ...
            connection.close()
        print("Database is already valid v15.")
        return 0
    ...
    backup = PROJECT_ROOT / f"trade_log_before_v15_{stamp()}.db"
    ...
    print("Database schema: v15")
    print("SQLite integrity_check: ok")
    print("Foreign-key check: clean")
    print()
    print(
        "Building an index on an existing listing_reference_contracts "
        "table can take a noticeable moment on a real database with "
        "many accumulated runs. That is expected and is a one-time cost."
    )
    return 0
