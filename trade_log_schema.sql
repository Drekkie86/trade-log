...
-- ---------------------------------------------------------------------
-- annotations — append-only. Later thinking lives here, never in the
-- prediction fields.
-- ---------------------------------------------------------------------
...
-- stops hindsight from rewriting the record. A trigger fires on UPDATE
-- OF the listed columns whether or not the value changed, so the app
-- must never include these in an UPDATE statement.
-- =====================================================================

-- Prediction and risk plan: fixed at entry.
CREATE TRIGGER trg_trades_prediction_immutable
BEFORE UPDATE OF
    thesis, prediction, horizon_date, p_thesis, p_profit, invalidation,
    max_loss, profit_target, stop_condition
ON trades
BEGIN
    SELECT RAISE(ABORT,
...
-- Entry market data and provenance: evidence, not state.
CREATE TRIGGER trg_trades_entry_immutable
BEFORE UPDATE OF
    created_at, entry_at, entry_underlying, entry_fx_rate,
    entry_fees, entry_cash, underlying, currency,
    parent_trade_id, strategy, rejection_reason
ON trades
BEGIN
    SELECT RAISE(ABORT,
...
-- Leg entry data: same rule as trade entry data.
CREATE TRIGGER trg_legs_entry_immutable
BEFORE UPDATE OF
    trade_id, leg_no, right, direction, strike, expiration,
    contracts, multiplier, entry_bid, entry_ask, entry_fill
ON trade_legs
BEGIN
    SELECT RAISE(ABORT, 'Leg entry data is immutable.');
END;
...
-- Trade decisions and legs are audit evidence: never physically delete them.
...
-- Voiding is a one-way audit action and must carry a timestamp and reason.
...
-- A rejected decision must not acquire legs.
...
-- Slippage against mid, per leg, both sides. Positive = paid away.
-- Buys cost you when fill > mid; sells cost you when fill < mid.
...
-- Calibration input. One row per resolved decision, per question.
-- 'THESIS' rows include rejected decisions; 'PROFIT' rows cannot.
...
-- Calibration by decile bucket. With single-digit n this is noise —
-- it is here so the shape exists, not because it means anything yet.
