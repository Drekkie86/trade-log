# Christiania Cohort 001 Preregistration v2
...
**Repository path:** `research/cohort_001_preregistration_v2.md`  
**Supersedes before-data v1:** git blob `af2033f3576b4b3789cdd142dd684489d6a4840b`
...
### 5.1 Selection-stage eligibility evidence

Every contract that successfully passes Massive normalization must enter the selection stage.

No normalized contract may silently disappear between normalization and stratification.

Every normalized contract must be assigned exactly one selection-stage state:
...
- its normalized option-quote row exists;
- it has a provider contract identifier or option symbol;
- exactly one Massive provider-model observation matches that normalized contract;
- that matched model observation contains a finite delta;
- `0.10 <= |delta| <= 0.80`.

An excluded normalized contract must be persisted with an explicit reason.
...
Contracts outside the preregistered absolute-delta range are expected sampling exclusions, not normalization failures.

The following run-level reconciliation is mandatory:

`normalized contracts = selection-eligible contracts + selection exclusions`

The selection-exclusion count and reason breakdown must be retained as research evidence.

A selection exclusion may not depend on:
...
5. if still tied, choose the lower immutable `option_quote_id`.

The fifth key exists only to make the ordering total and independent of provider page order.
...
- selection-exclusion count
- selection-exclusion reason breakdown
...
- normalized/selection-eligible/selection-exclusion counts do not reconcile
- a normalized contract disappears before selection without an exclusion row
...
```powershell
git hash-object research/cohort_001_preregistration_v2.md
```
...
**Sampling rule:** `BASELINE_STRATIFIED_SAMPLE_V2`
