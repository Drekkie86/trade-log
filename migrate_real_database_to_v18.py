...
MIGRATION_PATH = (
    PROJECT_ROOT
    / "migrations"
    / "018_evidence_integrity_instrumentation.sql"
)
EXPECTED_BEFORE = 17
EXPECTED_AFTER = 18
...
def verify(connection: sqlite3.Connection) -> None:
    if version(connection) != EXPECTED_AFTER:
        raise RuntimeError("Schema did not reach v18.")
    ...
    if missing:
        raise RuntimeError(
            f"Missing v18 provider-model timing columns: {sorted(missing)}"
        )
    ...
    for view_name in {
        ...
        if view_name not in objects:
            raise RuntimeError(f"Missing v18 diagnostic view: {view_name}")

    # Existing historical model rows remain valid and intentionally have NULL
    # timing diagnostics; v18 only instruments observations created afterward.
    connection.execute(
        "SELECT COUNT(*) FROM v_unmatched_provider_gap_by_run;"
    ).fetchone()
    connection.execute(
        "SELECT COUNT(*) FROM v_unmatched_provider_identity_recurrence;"
    ).fetchone()
...
def main() -> int:
    print("Christiania - real database migration to v18")
    print("---------------------------------------------")
    print()
    print(
        "This migration adds observational ThetaData Greek timing fields "
        "and derived unmatched-provider diagnostic views. It does not "
        "change V1 scanner/admission thresholds or rewrite historical evidence."
    )
    print()
    ...
    if current == EXPECTED_AFTER:
        ...
        print("Database is already valid v18.")
        return 0
    ...
    backup = (
        PROJECT_ROOT
        / f"trade_log_before_v17_{stamp()}.db"
    )
    ...
    print("Database schema: v18")
    print("SQLite integrity_check: ok")
    print("Foreign-key check: clean")
    print("Greek timing instrumentation: available")
    print("Reference-gap diagnostic views: available")
    return 0
