...
from migrate_real_database_to_v12 import (
    connect,
    sqlite_backup,
    stamp,
)
...
MIGRATION = (
    ROOT
    / "migrations"
    / "013_research_daemon_and_shadow_marks.sql"
)
...
def verify(
    ...
    required = {
        ...
        "trg_shadow_mark_observations_no_update",
    }
    ...
    if missing:
        raise RuntimeError(
            f"Missing v13 objects: {sorted(missing)}"
        )
...
def main() -> int:
    print(
        "Christiania - real database migration to v13"
    )
    print(
        "-------------------------------------------"
    )
    ...
    rehearsal = (
        ROOT / "trade_log_v13_rehearsal.db"
    )
    rehearsal.unlink(
        missing_ok=True
    )
    ...
    backup = (
        ROOT
        / f"trade_log_before_v13_{stamp()}.db"
    )
