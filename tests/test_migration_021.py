...
def test_v21_objects_and_firewall(db_path):
 c=get_connection(db_path); assert c.execute('SELECT MAX(version) FROM schema_version').fetchone()[0]==EXPECTED_SCHEMA_VERSION; names={r['name'] for r in c.execute("SELECT name FROM sqlite_master WHERE type IN ('table','view')")}; assert 'local_surface_robustness_v1_runs' in names and 'v_local_surface_robustness_v1_episode_summary' in names
 try:
