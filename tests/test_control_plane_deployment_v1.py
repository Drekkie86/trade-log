...
def test_receiver_uses_proven_release_ownership_without_ui_provisioning():
    script = _read(
        "deploy/receive_release.sh"
    )

    assert "provision_runtime_identities.sh" not in script
    assert "christiania_ui_env.py" not in script
    assert "validate_ui_readonly_runtime" not in script
    assert (
        'chown -R root:"${SERVICE_USER}" "${RELEASE_DIR}"'
        in script
    )
    assert 'RUNTIME_GROUP="christiania-runtime"' not in script
    assert 'UI_USER="christiania-ui"' not in script
...
def test_receiver_does_not_lock_current_backend_out_of_release_root():
    ...
    assert 'chmod 0750 "${RELEASE_ROOT}"' in script
    assert 'RUNTIME_GROUP="christiania-runtime"' not in script
