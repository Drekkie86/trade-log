...
if ! id "${UI_USER}" >/dev/null 2>&1; then
    useradd         --system         --home-dir /nonexistent         --shell /usr/sbin/nologin         --gid "${RUNTIME_GROUP}"         "${UI_USER}"
else
    usermod         --gid "${RUNTIME_GROUP}"         "${UI_USER}"
fi

install     -d     -m 0750     -o root     -g "${RUNTIME_GROUP}"     "${UI_ENV_ROOT}"

for directory in     "${STATE_ROOT}/data"     "${STATE_ROOT}/backups"     "${STATE_ROOT}/audit"
do
    install         -d         -m 2750         -o "${SERVICE_USER}"         -g "${RUNTIME_GROUP}"         "${directory}"

    chgrp -R         "${RUNTIME_GROUP}"         "${directory}"

    find "${directory}"         -type d         -exec chmod u+rwx,g+rx,o-rwx,g+s {} +

    find "${directory}"         -type f         -exec chmod u+rw,g+r,o-rwx {} +
done
